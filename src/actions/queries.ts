  ...
  const [partners, projectsRaw, drawingsRaw, companyRaw, fixedCostsRaw] = await Promise.all([
    ...
    db.companyExpense.findMany({
      ...
      select: {
        ...
        payouts: { select: { partnerId: true, amount: true, expenseId: true, paidByPartnerId: true } },
      },
      orderBy: { expenseDate: "desc" },
    }),
    ...
    expenses: companyRaw.map((e) => ({
      ...
      payouts: e.payouts
        ...
        .map((x) => ({
          ...
          paidByPartnerId: x.paidByPartnerId ?? undefined,
        })),
    })),
...
export async function getSummaryData() {
  ...
  return {
    ...
    projectCosts: projectCosts.map((c) => ({
      ...
      deductFromCustody: c.deductFromCustody,
      project: c.project,
    })),
    partners,
  };
