/**
 * Neon DB health check — read-only verification.
...
async function main() {
  ...
  console.log("→ Connecting to Neon (pooled DATABASE_URL)…");
  ...
  console.log(`✓ Neon health check passed in ${Date.now() - started}ms`);
}

main()
  .catch((e) => {
    console.error("✗ Neon health check FAILED:", e);
    process.exit(1);
  })
